...
cases_filtered_encoded <- cases_filtered |>
  select(`Account Number`, `Case Age (Days)`, `Is Escalated`, Site, `Product Category`, `Status Reason.y`) |>
  mutate(churned = ifelse(`Status Reason.y` %in% churn_statuses, 1, 0)) |>
  dummy_cols(select_columns = c("Site", "Product Category"))

glimpse(cases_filtered_encoded)
...
```{r}
lower_bound_num <- Q1 - 1.5 * IQR_num
upper_bound_num <- Q3 + 1.5 * IQR_num
...
```{r}
cor(customer_features_clean |> select(num_cases, avg_case_age, churned))
...
```{r}
linear <- lm(churned ~ num_cases + case_age_ratio, data = customer_features_clean)
summary(linear)
```
### Logistic Regression

```{r}
log_model <- glm(churned ~ num_cases + case_age_ratio, data = customer_features_clean, family = binomial)
summary(log_model)

```
```{r}
lm_predictions <- predict(linear, customer_features_clean)

log_predictions <- predict(log_model, customer_features_clean)

roc_lm <- roc(customer_features_clean$churned, lm_predictions)

roc_log <- roc(customer_features_clean$churned, log_predictions)

```
```{r}
ggroc(list(Linear_Regression = roc_lm, Logistic_Regression = roc_log)) +
