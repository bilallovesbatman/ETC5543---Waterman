...
train_indices <- createDataPartition(customer_features_clean$churned, p = 0.7, list = FALSE)

# Split the data
train <- customer_features_clean[train_indices, ]
test <- customer_features_clean[-train_indices, ]

```


```{r}
log_model <- glm(churned ~ num_cases + case_age + is_escalated + site + num_cases_escalated + case_age_escalated, data = train, family = binomial)
summary(log_model)

```

Since `site` is not a significant predcitor in our case and is complex to handle, we decide to remove it.

```{r}
log_model_no_site <- glm(churned ~ num_cases + case_age + is_escalated + num_cases_escalated + case_age_escalated, 
                          data = train, family = binomial)
summary(log_model_no_site)

```


```{r}
pscl::pR2(log_model_no_site)["McFadden"]
```

```{r}
varImp(log_model_no_site)
```

```{r}
car::vif(log_model_no_site)
```

### Model Diagnostics

```{r}
predicted_no_site <- predict(log_model_no_site, test, type = "response")
predicted_churned_no_site <- ifelse(predicted_no_site > 0.5, 1, 0)
predicted_churned_no_site <- factor(predicted_churned_no_site, levels = c("0", "1"))

```

```{r}
confusionMatrix(predicted_churned_no_site,
                as.factor(test$churned))
```

```{r}
roc_curve <- roc(test$churned, predicted_no_site)

plot(roc_curve, main = "ROC Curve", col = "blue", lwd = 2)

auc_value <- auc(roc_curve)
legend("bottomright", legend = paste("AUC =", round(auc_value, 3)), box.lwd = 0, col = "blue", lwd = 2)
```
