...
Using the defined churn statuses, the joined dataset was further filtered to include only the churned customers and a binary variable `Is Escalated` is created to provide numeric representation to of case escalation. Here, when a case is marked "Yes", the final value is converted to `1` and `0` otherwise.
...
```{r}
vis_dat(customer_features)

vis_miss(customer_features)
```
