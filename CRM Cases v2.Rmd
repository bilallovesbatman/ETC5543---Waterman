...
The merged data produced a large dataset with an increasing number of variables that needed to be cleaned further. As before, potential `NA` values were catered for and further nonessential variables and observations were removed. The resulting dataset had the following variables:
...
kable(variable_list4, 
      caption = "Variables in the `customer_features` Dataset", 
      align = "l")
```
...
- `avg_case_age`: The median age in days taken to resolve a case.
...
```{r vis-plot, fig.cap="Comparison of dataset structure and missing values.", echo=FALSE, fig.width=8, fig.height=5}
...
While we are able to what type the variables are, \@ref(fig:vis-plot) shows that there are a significant number of missing observations in `product_category`. These missing values pose a problem when it comes to analyzing the product categories and a decision was taken to exclude the variable altogether for our analysis.

### Distribution of Case Frequency and Case Age

```{r outlier-plot, fig.cap= "Outlier Distributions", echo=FALSE, fig.width=8, fig.height=5}
...
his2 <- ggplot(customer_features, aes(x = avg_case_age, fill = as.factor(churned))) +
  geom_histogram(position = "dodge", binwidth = 1) +
...
\@ref(fig:outlier-plot) reveals positive
